...
def execute(filters=None):
    ...
    if filters.get("customer_name"):
        conditions += " and `tabService Report`.customer_name = %s" % frappe.db.escape(filters.get("customer_name"))
...
#
    ...
    qry= """Select `tabItem`.item_class, `tabMaterial Consume Table`.item_name ,`tabMaterial Consume Table`.item_code , sum(`tabMaterial Consume Table`.quantity),`tabItem`.valuation_rate*sum(`tabMaterial Consume Table`.quantity) from `tabMaterial Consume Table` Join `tabService Report` ON `tabService Report`.name = `tabMaterial Consume Table`.parent Join `tabItem` ON `tabMaterial Consume Table`.item_code = `tabItem`.item_code where 1=1 """+ conditions + """ GROUP BY `tabMaterial Consume Table`.item_name  ORDER BY  `tabMaterial Consume Table`.item_name"""
    #frappe.throw(qry)
